...
def run():
    ...
    while step < 500:
        try:
            ...
            if(step >= 40 and step <= 100):
                #traci.lane.setAllowed("exit_0", ["emergency"])
                #traci.lane.setAllowed("-exit_0", ["emergency"])
            ...
            for vehicle in traci.vehicle.getIDList():
                ...
                '''
                if "J" in str(entity['current_edge']):
                    entity['in_junction'] = {"val": 1, "err":0}
                else:
                    entity['in_junction'] = {"val": 0, "err":0}
                ...
                #entity['tl_distance'] = round(traci.vehicle.getNextTLS(vehicle)[0][2])
...
if __name__ == "__main__":
    ...
    traci.start([sumoBinary, "-c", "./2.sumocfg.xml",
                             "--tripinfo-output", "merge_info.xml", "--fcd-output", "merge_trace.xml","--lanechange-output", "merge_lanechange_output.xml", "--seed", "1111", "--device.rerouting.period", "100"])
